...
struct InventoryFormView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .confirmationAction) {
                ...
                .disabled(vm.loadingState != .none || vm.name.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines).isEmpty)
            }
        }
        ...
        .fileImporter(isPresented: .init(get:{ vm.selectedUSDZSource == .fileImporter}, set: {
            _ in vm.selectedUSDZSource = nil
        }), allowedContentTypes: [UTType.usdz], onCompletion: {
            result in
            switch result {
            case .success(let url):
                Task {
                    await vm.uploadUSZD(fileURL: url)
                }
        ...
        .alert(isPresented: .init(get: {
            ...
            _ in vm.error = nil
        }),error: "An error has occured", actions: { _ in Text(vm.error ?? "")}, message: { _ in
            Text(vm.error ?? "")
        })
    ...
    func viewAR(url: URL) {
        ...
        let vc = UIApplication.shared.firstKeyWindow?.rootViewController?.presentedViewController ??
